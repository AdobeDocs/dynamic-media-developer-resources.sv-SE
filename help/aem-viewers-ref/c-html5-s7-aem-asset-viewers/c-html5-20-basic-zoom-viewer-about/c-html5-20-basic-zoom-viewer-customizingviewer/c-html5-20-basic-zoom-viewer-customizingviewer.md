...
Tänk på att visningsprogrammet tilldelar `.s7basiczoomviewer` till behållar-DOM-elementet. Om du använder en extern CSS-fil som skickas med `style=` kommando, använda `.s7basiczoomviewer` som överordnad klass i underordnad väljare för dina CSS-regler. Om du gör inbäddade format på webbsidan kvalificerar du även den här väljaren med ett ID för behållar-DOM-elementet enligt följande:
...
Den andra gruppen CSS-markörer innehåller `.s7mouseinput` och `.s7touchinput`. The `.s7touchinput` markör anges om den aktuella enheten har funktioner för pekrörelser, i annat fall, `.s7mouseinput` används. Dessa markörer är främst avsedda att skapa indataelement i användargränssnittet med olika skärmstorlekar för olika indatatyper, eftersom pekrörelser normalt kräver större element. Om enheten har både musingång och pekfunktioner `.s7touchinput` är inställt och visningsprogrammet återger ett pekvänligt användargränssnitt.
