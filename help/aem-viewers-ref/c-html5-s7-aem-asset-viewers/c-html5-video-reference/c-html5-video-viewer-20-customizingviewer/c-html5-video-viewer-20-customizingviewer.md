...
Kom ihåg att visningsprogrammet tilldelar `.s7videoviewer` till behållar-DOM-elementet. Om du använder en extern CSS-fil som skickas med `style=` kommando, använda `.s7videoviewer` som överordnad klass i underordnad väljare för dina CSS-regler. Om du bäddar in format på webbsidan kvalificerar du även den här väljaren med ett ID för behållar-DOM-elementet enligt följande:
...
I följande exempel på CSS anges storleken på uppspelnings-/pausknappen till 28 x 28 pixlar på system med musindata och 56 x 56 pixlar på pekenheter. Dessutom döljs knappen helt om visningsprogrammets storlek blir för liten:
...
Att använda CSS-markörer är det mest flexibla sättet att bygga responsiv CSS. Orsaken är att den gör att du inte bara kan ange enhetens skärmstorlek som mål, utan även den faktiska visningsstorleken, vilket kan vara användbart för responsiva layouter för designsidor.
