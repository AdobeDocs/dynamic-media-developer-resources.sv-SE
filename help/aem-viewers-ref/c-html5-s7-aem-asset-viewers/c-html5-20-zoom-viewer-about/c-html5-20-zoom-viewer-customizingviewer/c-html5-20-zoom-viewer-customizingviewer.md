...
# Anpassa Zoom Viewer{#customizing-zoom-viewer}
...
Tänk på att visningsprogrammet tilldelar `.s7zoomviewer` till behållar-DOM-elementet. Om du använder en extern CSS-fil som skickas med `style=` kommando, använda `.s7zoomviewer` som överordnad klass i underordnad väljare för dina CSS-regler. Om du gör inbäddade format på webbsidan kvalificerar du även den här väljaren med ett ID för behållar-DOM-elementet enligt följande:
