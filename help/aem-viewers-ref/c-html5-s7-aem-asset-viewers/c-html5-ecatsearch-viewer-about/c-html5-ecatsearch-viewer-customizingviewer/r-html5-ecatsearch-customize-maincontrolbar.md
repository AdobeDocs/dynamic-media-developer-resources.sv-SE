...
>[!NOTE]
>
>Den här knappen stöder attributväljarna `state` och `selected` som kan användas för att tillämpa olika skal på olika knapplägen. I synnerhet motsvarar `state="selected"` det inledande rullningsknappläget när det är möjligt att rulla innehållet i kontrollfältet åt vänster. `state="default"` motsvarar läget när innehållet rullas hela vägen till vänster och rullningsknappen föreslår att det återgår till det ursprungliga läget.
