...
>[!NOTE]
>
>Den här knappen har stöd för både `state` - och `selected` attributväljare, som kan användas för att tillämpa olika skal på olika knapplägen. Motsvarar i synnerhet `selected='true'` visningsläget när miniatyrbildsläget är aktivt och `selected='false'` motsvarar standardläget med huvudvyn.
