...
keywords: responsive
seo-description: All visuell anpassning och de flesta beteendeanpassningar för Carousel Viewer görs genom att en anpassad CSS skapas.
seo-title: Anpassa Carousel Viewer
...
När du skapar anpassad CSS bör du tänka på att visningsprogrammet tilldelar `.s7carouselviewer` klassen till dess behållar-DOM-element. Om du använder en extern CSS-fil som skickas med `style=` kommandot använder du `.s7carouselviewer` klassen som överordnad klass i den underordnade väljaren för dina CSS-regler. Om du lägger till inbäddade format på webbsidan kan du även kvalificera den här väljaren med ett ID för behållar-DOM-elementet enligt följande:
