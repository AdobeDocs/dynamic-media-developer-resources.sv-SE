...
Klammerparentes `{ }` och parenteser `( )` kan användas för att avgränsa visst innehåll (till exempel färgvärden) eller för att ange kapslade uppsättningar. Klammerparenteser och parenteser som används på det här sättet får inte kodas och måste alltid visas som matchade par, annars inträffar ett katalogtolkningsfel.
...
Textsträng. Kommaavgränsad lista med `catalog::Id` värden, absoluta sökvägar för Image Server-filer eller sökvägar i förhållande till `attribute::RootPath`. Samma bild kan refereras mer än en gång i uppsättningen. Den definierande katalogposten kan visas i uppsättningen på vilken plats som helst.
