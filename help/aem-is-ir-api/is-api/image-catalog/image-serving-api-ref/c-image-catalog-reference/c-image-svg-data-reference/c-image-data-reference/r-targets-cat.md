...
Servern returnerar innehållet i det här fältet som svar på `req=targets`, efter ersättning av `??`&#39; postterminatortoken.

Upp till fyra egenskaper kan kopplas till varje zoommål:

` Target. *`num`*.frame= *`frame`*`

` Target. *`num`*.rect= *`vänster,övre,bredd,höjd`*`
...
*`label`* och *`userData`* delta i textsträngslokalisering. Se [Lokalisering av textsträng](/help/aem-is-ir-api/is-api/http-ref/image-serving-api-ref/c-http-protocol-reference/c-syntax-and-features/r-text-string-localization.md) i *HTTP-protokollreferens* för mer information.
...
Visningsprogrammen i Dynamic Media förväntar sig zoommål i koordinaterna för den högupplösta bilden som redan har justerats med kommandona från `catalog::Modifier`.

## Egenskaper {#section-b3f8eba4985f4b00bb935d592fe770f9}

[Egenskapsdata](/help/aem-is-ir-api/is-api/image-catalog/image-serving-api-ref/c-image-catalog-reference/c-overview/c-common-data-types/r-property-data.md) värde.
