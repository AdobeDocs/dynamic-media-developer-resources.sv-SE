...
Image Serving är en mekanism för att hantera innehåll som inte finns i bilder i kataloger och hantera det via en separat `context /is/content`. Mekanismen gör det möjligt att konfigurera TTL för varje objekt separat.
...
Den här mekanismen kan hjälpa till att säkerställa att klienter endast får det innehåll som passar deras behov. Om det statiska innehållet är taggat med rätt `catalog::UserType`värden kan klienten lägga till `type=` kommandot i begäran. Bildrutefunktionen jämför värdet som anges med `type=` kommandot med värdet för `catalog::UserType` och returnerar ett fel i stället för eventuellt olämpligt innehåll om det inte matchar.
