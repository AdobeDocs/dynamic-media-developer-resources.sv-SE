...
Uppsättningssvaret för en `catalog::ImageSet` kan hämtas med modifieraren `req=set` och med en referens till katalogpost-ID:t i nätsökvägen. Alternativt kan bilduppsättningen anges direkt i URL:en med modifieraren `imageset=`. Om modifieraren `imageset=` används för att ange bilduppsättningen bör hela värdet omges av klammerparenteser för att undvika bilduppsättningsvärdet och för att säkerställa att alla inkluderade modifierare inte tolkas som en del av URL-frågesträngen.
...
När en `req=set`-begäran tas emot bestäms typen av svar som ska genereras av värdet `catalog::AssetType`. Om `catalog::AssetType` inte definieras bestäms svarstypen av följande regler:

* Om det finns poster i bildkatalogen, definieras AND `catalog::ImageSet`
...
För att begränsa storleken på xml-svaren för stora e-kataloguppsättningar, inaktiveras privata metadata för broschyruppsättningar enligt serveregenskapen `PS::fvctx.brochureLimit`. Alla privata metadata som är associerade med broschyren exporteras tills broschyrgränsen nås. När gränsen har överskridits ignoreras privata kartor och användardata och en motsvarande flagga ställs in för att ange vilken typ av data som har utelämnats.
