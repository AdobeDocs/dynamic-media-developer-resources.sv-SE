...
Förutom att hämta bilder med exakta upplösningar kan `res=` även användas för att kombinera flera bilder med samma upplösning, så att de objekt som visas i de här bilderna har rätt proportioner i förhållande till varandra.
...
Source image/mask-attribut. Ignoreras av lager som inte är kopplade till en källbild eller källmask. Tillämpas på lager 0 har angetts för `layer=comp`. Ignoreras om antingen `scale=` eller `size=` har angetts för samma lager.

## Standard {#section-c5f1ba6fe53d46eca32e7d0588dcdf3d}

Om inget anges avgör `scale=` eller `size=` skalningsfaktorn, eller om inget anges används bilden utan skalning.
