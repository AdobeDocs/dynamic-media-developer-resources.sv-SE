...
Begär attribut. Servern returnerar ett fel om en bildtyp har angetts som inte matchar `fmt=` den *`profile`*.

*`renderIntent`* och ignoreras *`blackpointComp`* om de inte är kompatibla med den angivna ICC-profilen. Det är mer sannolikt att profiler för CMYK-utdataenheter har stöd för olika återgivningsmetoder.
