...
Nyckelordet gör det möjligt att uppdatera cacheposter när bildfiler har ändrats, utan att behöva vänta på att cacheposten ska upphöra automatiskt. `validate` Klientcachning påverkas inte av det här kommandot.
...
Begär attribut. Används oavsett aktuell lagerinställning. Ignoreras när begäran inte returnerar en svarsbild. *`clientControl`*ignoreras när cachelagring på klientsidan inaktiveras av bildkatalogen (om `catalog::Expiration` den har ett negativt värde).

Cachekontroll på klientsidan ( `on` och `off` endast) är även tillgänglig för statiska innehållsbegäranden på [!DNL /is/content/].
