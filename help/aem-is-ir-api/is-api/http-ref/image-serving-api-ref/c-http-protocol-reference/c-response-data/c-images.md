...
MIME-typen för HTTP-svar bestäms av `fmt=`, eller om `fmt=` har inte angetts, den är `<image/jpeg>`.

HTTP-svarsstatusen är &quot;200 OK&quot; om förfrågningsmetoden var ovillkorlig `GET` eller `HEAD`.

Servern kan svara med statusen 304 (inte ändrad) och returnera inga bilddata som svar på ett villkorligt `GET` begäran (som innehåller en giltig `If-Modified-Since` eller `If-None-Match` header).
