...
Bilden läggs inte till om både `wid=` och `hei=` anges och bredd/höjd skiljer sig från bildens proportioner.
...
Kan inträffa var som helst i begäran. Om du ändrar storlek på bilden med `wid=`, `hei=`eller `scl=` inte ändras värdet för utskriftsupplösning som är inbäddat i svarsbilden. Ignoreras om `scl=` inträffar efter `wid=` och/eller `hei=` i kommandosekvensen.
