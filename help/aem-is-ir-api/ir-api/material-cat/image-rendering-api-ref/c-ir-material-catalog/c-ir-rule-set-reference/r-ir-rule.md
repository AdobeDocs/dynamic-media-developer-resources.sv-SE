...
` Name=" *`valfri text`*"` . Används för att identifiera `<rule>` elementet i felsökningsloggar och felmeddelanden.

Elementen kan dessutom `<rule>` definiera följande attribut i valfri kombination. Om den anges och regeln matchas utan fel åsidosätter de motsvarande katalogattributen för den här begäran.
...
Om `<expression>` inte anges kommer alla sökvägar att matcha och `<substitution>` läggas till i slutet av sökvägen.
