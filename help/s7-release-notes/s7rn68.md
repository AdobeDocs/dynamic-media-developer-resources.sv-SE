...
description: "Den senaste versionsinformationen som beskriver de senaste funktionerna, förbättringarna och fixarna för Adobe Scene7 Publishing System 6.8, som ingår i Adobe Experience Manager-lösningen i Adobe Experience Cloud."
...
Den senaste versionsinformationen som beskriver de senaste funktionerna, förbättringarna och korrigeringarna för [!DNL Adobe Scene7 Publishing System] 6,8-delar av Adobe Experience Manager-lösningen i [!DNL Adobe Experience Cloud].
...
* Lägsta systemkrav för macOS är nu macOS 10.8.
...
* Förutom möjligheten att överföra innehåll till Scene7 Publishing System via FTP (server directory upload) och ladda upp URL:er, kan du nu även ladda upp innehåll med HTTP-POST. Se [API för bildproduktionssystem](/help/aem-ips-api/c-overview.md) om du vill ha mer information om hur du använder HTTP-POST.
...
* HTML5 eCatalog Viewer - dra-och-släpp fungerar inte i Internet Explorer när sidan innehåller bildscheman.
...
* autoResize ska inte finnas i HTML i visningsprogramkod för text i text-/inbäddningsversion.
...
* &quot;Space After&quot; fungerar inte; bör använda `sa`, för närvarande är det `margb`.
* När företagsinställningarna initieras skickar Scene7 Publishing System ett dubblettalternativ `<defaultValue>` i API `<createMetadataField>`.

* Plugin-programmet Flash och Scene7 Publishing System kraschade på grund av att många mappar inte har tillräckligt med Flash-minne.
...
* Uppdateringen av Adobe Illustrator och Adobe InDesign från maj 2014 förväntas medföra inkompatibilitet med plugin-program som skapar Scene7 FXG. Dessa inkompatibiliteter påverkar användare av Scene7 web-to-Print-lösning. Om du använder Scene7 Web-to-Print ska du inte uppgradera Illustrator och InDesign till denna Creative Cloud-uppdatering. Använd istället före maj 2014-versionen. Adobe Inc förväntar sig att kompatibiliteten för Scene7 FXG-skapande med nyare versioner av Illustrator och InDesign återställs under hösten 2014.
...
* Från om med maj 2014 har Akamai inte längre stöd för e-postmeddelanden när Cacheinvalidering i Scene7 har slutförts. Den vanliga bearbetningstiden för CDN-ogiltigförklaring är ungefär åtta minuter. Om det finns en stor kö med förfrågningar, eller många URL:er, kan bearbetningstiden ta något längre tid.
...
* Sociala funktioner som visas i helskärmsläge i visningsprogram för HTML5.
...
* Fast: Behållarkoordinaterna är felaktiga om behållaren flyttas med CSS-omvandling.
...
* Fast: Ikonen för delning via sociala medier tonas inte ut med `controlbar` på Zoom.
...
* iPhone videoproxy har implementerats för att åtgärda videoproblem på iPhone-enheter.
...
* Fast: Artefakter visas när du har zoomat in i bilden eller roterat en enhet på Android™.
