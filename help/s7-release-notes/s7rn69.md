---
description: Den senaste releaseversionen som beskriver de senaste funktionerna, förbättringarna och fixarna för Adobe Scene7 Publishing System 6.9 i Adobe Experience Manager-lösningen i Adobe Marketing Cloud.
seo-description: Den senaste releaseversionen som beskriver de senaste funktionerna, förbättringarna och fixarna för Adobe Scene7 Publishing System 6.9 i Adobe Experience Manager-lösningen i Adobe Marketing Cloud.
...
Den senaste releaseversionen som beskriver de senaste funktionerna, förbättringarna och fixarna för Adobe Scene7 Publishing System 6.9 i Adobe Experience Manager-lösningen i Adobe Marketing Cloud.
...
* **SSO-kapacitet (Single Sign-On)** - Nu kan du starta Scene7 från Adobe Marketing Cloud och använda Adobe ID för att logga in på Scene7.

   Om du vill ha SSO-konfiguration så att du kan använda den med Scene7 kontaktar du Adobes tekniska support. Ha följande information klar så att era Marketing Cloud- och Scene7-konton kan länkas samman: Organisationens namn i Adobe Marketing Cloud. ditt Scene7-företagsnamn, och din geografiska region.

## Bildbehandling {#section-98a61ea9a09940639f357f36969d6e55}

* **Bättre stöd** för responsiva bilder - Optimera kvaliteten ytterligare för responsiva bildupplevelser genom att definiera olika förinställningar eller modifieringar för varje brytpunkt. Se [Använda responsivt statiskt bildbibliotek](https://docs.adobe.com/content/help/en/dynamic-media-developer-resources/image-serving-api/image-serving-api/responsive-static-image-library/c-about-responsive-static-image-library.html).
...
* Videobildtexter finns nu i färdiga `Universal_HTML5_MixedMedia_dark` och `Universal_HTML5_MixedMedia_light` färdiga visningsprogram. Se [Lägga till bildtexter till video](https://docs.adobe.com/content/help/en/dynamic-media-classic/using/video/adding-captions-video.html).

## Webb-till-utskrift {#section-17c5591d279a48d3b0aea085a19d6321}

* **Adobe Illustrator Plug-in for Web-to-Print och Adobe InDesign Plug-in for Web-to-Print** - Dessa två nya plugin-program ersätter den tidigare CSXS-pluginen (Creative Suite Extension) för Web-to-Print i Scene7. Se [Snabbstart: Mallpublicering](http://help.adobe.com/en_US/scene7/using/WS447286D6-49D9-427c-906A-3A8169974681.html).
...
* Övervakning av funktionen för ogiltig CDN-cache - När du har valt en CDN-valideringsmall (se [Allmänna inställningar](http://help.adobe.com/en_US/scene7/using/WSCCBA9D3A-06A3-4f29-AF6B-36CBB2A655F1.html)**[!UICONTROL Invalidate CDN]** ) och en begäran görs via Arkiv-menyn i SPS visas nu en indikator i användargränssnittet som ger dig en uppskattning av hur lång tid det tar att rensa cachen.
...
* Stöd för `setCSS()`, `setModifier()`och `dispose()` metoder har lagts till.
* Stöd för `getWidth()`och `getHeight()` metoder har lagts till.
* Stöd för `setPortrait()` och `setLandscape ()` metoder har lagts till.
* Fixed - IE9: s7sdk.event: PageMouseEvent: klicka och dubbelklicka på händelser som utlöser en krasch i webbläsaren.
* Tillagd `TableOfContents` komponent.
...
* Fixed - bilden har flyttats till höger om den har `DIV` en `text-align:center` stil.
...
**Nya funktioner, förbättringar och felkorrigeringar för Scene7 Image Serving Viewers 5.1.1**
...
Se även [Referenshandbok](https://docs.adobe.com/content/help/en/dynamic-media-developer-resources/library/home.html)för Scene7-visningsprogram.
...
* Metadata Server och Likhetssökning kommer båda att bli inaktuella under 2015 på grund av begränsad användning. När det officiella måldatumet för borttagning tillkännages kommer berörda kunder att meddelas av Adobes tekniska support.
